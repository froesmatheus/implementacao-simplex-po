function condicaoParada(p_matriz) {
		...
		for (i = 1; i <= nLinhas; i++) {
			...
			if (i != indMenor) {
				for (j = 1; j <= nColunas; j++) {
					p_matriz[i][j] = parseInt(p_matriz[i][j]) + parseInt(-1 * aux * p_matriz[indMenor][j]);
				}
				printTabela(p_matriz);
			}
		}
	...
	function esconder(p_variaveis, p_restricoes) {
		for (i = 1; i <= p_variaveis; i++) {
			//document.getElementById('y'+i).style.outline = "0";
		...
		for (j = 1; j <= p_restricoes; j++) {
			//document.getElementById('b'+j).style.outline = "0";
